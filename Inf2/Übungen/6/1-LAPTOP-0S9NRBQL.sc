...
def equal2(code1:BitElem,code2:BitElem):Boolean={
  var p=code1
  var q = code2
  do{
    ...
    p=p.next
    q=q.next
  }while(p!=code1 || q!=code2)
  true
}
//equal2(string2Code("0"),string2Code("1"))
/*
...
def printCoinSet(coinSet : CoinElem) : Unit={
  //brauche iwie durchlauf dings
  var durch = coinSet
  //gehe einfach jedes durch und gebe die scheiße aus
  while(durch!= null){
...
def addCoin(coinset : CoinElem, coin : Coin) : Unit = {
  //wenn nur dummy drin ist extra abbacken
  if(coinset.next==null) {
    coinset.next=new CoinElem(coin,null)
  }else {
    var durch = coinset.next
    //führt mich ans ende der struktur
    while (durch.next != null) {
      durch = durch.next
    }
    //kann dann eintspannt neues einsetzen
...
def getValue(coinset : CoinElem, code : BitElem) : Int = {
  ...
  //gehe alles durch
  while(durch!= null){
    //schaue ob beide codes gleich sind / normiere vorher
    //in durch.coin.code steckt der code auf den ich vergleiche
    if(equal2(normCoinCode(durch.coin.code),normCoinCode(code))) out = durch.coin.value
    durch=durch.next
  }
  return out
}
